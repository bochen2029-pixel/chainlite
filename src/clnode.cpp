// chainlite node — a full validating node: P2P gossip, CPU miner, HTTP RPC,
// SQLite persistence. Run three of these on loopback and you have a network.
//
...
static HttpResp handle_rpc(App& a, const HttpReq& r) {
    ...
    // Serve the web viewer (block explorer + wallet) at / and /app.
    if (r.path == "/" || r.path == "/app" || r.path == "/viewer.html") {
        ...
        return { 200, "text/plain",
            "chainlite node RPC (web viewer not found; expected " + a.webroot + "\\viewer.html)\n"
